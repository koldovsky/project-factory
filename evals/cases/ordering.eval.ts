// REFERENCE EVAL CASES — graded quality scenarios (output evals).
//
...
//   - In a REAL project, `produce()` drives the running app: reuse the
//     Playwright harness from scripts/record-demos.reference.ts for UI cases,
//     or call the domain service directly for non-UI cases.
//   - The example cases below INLINE a sample output so this suite is
//     runnable in the framework repo, which has no app.
//
// TRACEABILITY
...
export const cases: EvalCase[] = [
  {
    ...
    rubric: [
      'CRITICAL: the error is shown inline next to the quantity field, never a generic 500 page',
  ...
  {
    id: 'eval-empty-state-no-orders',
    trace: ['NFR-7'],
    dimension: 'usability-clarity',
    capability: 'ordering',
    scenario: 'A newly provisioned customer opens the Orders list with zero orders.',
    // Real version: navigate to the empty list and return the empty-state region.
    produce: async () => ({
      httpStatus: 200,
      renderedEmptyState:
        '<div class="empty-state"><h2>No orders yet</h2>' +
        '<p>When you place your first order it will appear here.</p>' +
        '<a class="btn-primary" href="/orders/new">Create your first order</a></div>',
    }),
    rubric: [
